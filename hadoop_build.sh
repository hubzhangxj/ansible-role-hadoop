...
bench_path=${curdir##*/}
bench_role_git='ansible-role-hadoop'
bench_role_galaxy='hubzhangxj.hadoop'
###################################################################################
# Usage
###################################################################################
Usage()
...
benchmark_role()
{
	...
	sed -i '5s/ansible-role-hadoop/hubzhangxj.hadoop/'  ${default_site}
    elif [ "$bench_path" == "$bench_role_git" ];then
	echo "${bench_role_git} necessary to replace of ${bench_role_galaxy} "
	sed -i '5s/hubzhangxj.hadoop/ansible-role-hadoop/'  ${default_site}
    fi
}
